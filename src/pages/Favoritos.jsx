...
export default function Favoritos() {
  ...
  const cargarFavoritos = async () => {
    ...
    try {
      ...
      setFavoritosPersonalizados(
        favoritosDB.map((f) => ({...f, tipo: "personalizado"})),
      );
    } catch (error) {
  ...
  const confirmarEliminarFavorito = (numero, titulo, tipo) => {
    setModalEliminar({
      visible: true,
      himno: {identificador: numero, titulo, tipo},
    });
  };
    ...
    try {
      if (himno.tipo === "moravo") {
        ...
        setFavoritosHimnos((prev) =>
          prev.filter((h) => h.numero !== himno.identificador),
        );
        ...
        setFavoritosPersonalizados((prev) =>
          prev.filter((f) => f.id !== himno.identificador),
        );
        ...
        {totalFavoritos === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            <p className="text-slate-600 text-xs mb-4">
              Marca himnos o versículos como favoritos para verlos aquí
            </p>
            <div className="flex flex-wrap gap-2 justify-center">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-600/50 rounded-lg text-xs text-slate-300 transition-colors"
              >
                <FaMusic className="text-[10px]" /> Vida Cristiana
              </button>
            </div>
          </div>
        ) : (
      ...
      {vistaActual === "tarjetas" ? (
        <div className="p-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {favoritos.map((f) => (
            <div
              key={f.id}
              className="group bg-slate-800/60 border border-slate-700/50 hover:border-yellow-500/30 rounded-lg p-3 transition-colors"
            >
              ...
              >
                <FaPlay className="text-[10px]" /> Proyectar
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col">
          {favoritos.map((f) => (
            <div
              key={f.id}
              className="flex items-center gap-3 px-3 py-2 border-b border-slate-800/60 last:border-0 hover:bg-slate-800/40 transition-colors"
            >
...
const FavoritoCard = ({favorito, colorScheme, onNavigate, onEliminar}) => {
  ...
  const accentColor =
    ...
    "text-slate-400 border-slate-600/40 hover:border-slate-500/40";
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        <span
          className={`text-[10px] font-bold tabular-nums ${accentColor.split(" ")[0]}`}
        >
          #{identificador}
        </span>
        <button
      ...
      {favorito.autor && (
        <p className="text-[10px] text-slate-500 mb-2">{favorito.autor}</p>
      )}
...
const FavoritoListItem = ({favorito, colorScheme, onNavigate, onEliminar}) => {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 border-b border-slate-800/60 last:border-0 hover:bg-slate-800/40 transition-colors">
      <span
        className={`shrink-0 text-[10px] font-bold tabular-nums w-8 text-center ${accentClass}`}
      >
        {identificador || "—"}
      </span>
      <div
        ...
        onClick={() => onNavigate(identificador, favorito.tipo)}
      >
        <p className="text-sm text-slate-300 hover:text-white truncate transition-colors">
          {favorito.titulo}
        </p>
        {favorito.autor && (
