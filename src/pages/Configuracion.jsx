...
const inputCls =
  "w-full px-3 py-2 bg-slate-800 border border-slate-600/60 hover:border-slate-500 focus:border-indigo-500/70 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none transition-colors";
const selectCls =
  "w-full px-3 py-2 bg-slate-800 border border-slate-600/60 hover:border-slate-500 focus:border-indigo-500/70 rounded-lg text-sm text-white focus:outline-none transition-colors";
...
const Configuracion = () => {
  ...
  const guardarConfiguracionCompleta = async (configToSave = configuracion) => {
    ...
    const configAplanada = {
      ...
      mostrarLogo: (configToSave.mostrarLogo !== undefined
        ? configToSave.mostrarLogo
        : true
      ).toString(),
      mostrarNombreIglesia: (configToSave.mostrarNombreIglesia !== undefined
        ? configToSave.mostrarNombreIglesia
        : true
      ).toString(),
      mostrarEslogan: (configToSave.mostrarEslogan !== undefined
        ? configToSave.mostrarEslogan
        : true
      ).toString(),
    };
    const resultado =
              ...
              <div className="space-y-3">
                  ...
                  {[
                    ...
                    <div key={tipo}>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          actualizarFontSize(tipo, e.target.value)
                        }
                        className={selectCls}
                      >
                        {(tipo === "eslogan"
                          ? opcionesFontSize.slice(0, 4)
                          : opcionesFontSize
                        ).map((op) => (
                          <option
                            key={op.valor}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      actualizarCampo("logoSize", e.target.value)
                    }
                    className={selectCls}
                  >
                    <option value="w-56 h-56" className="bg-slate-800">
                      Pequeño
                    </option>
                    <option value="w-64 h-64" className="bg-slate-800">
                      Mediano
                    </option>
                    <option value="w-72 h-72" className="bg-slate-800">
                      Grande
                    </option>
                    <option value="w-80 h-80" className="bg-slate-800">
                      Muy Grande (recomendado)
                    </option>
                    <option value="w-96 h-96" className="bg-slate-800">
                      Extra Grande
                    </option>
                    <option
                      value="w-[28rem] h-[28rem]"
                      className="bg-slate-800"
                    >
                      Gigante
                    </option>
                    <option
                      value="w-[32rem] h-[32rem]"
                      className="bg-slate-800"
                    >
                      Super Gigante
                    </option>
              ...
              {!configuracion.nombreIglesia && !configuracion.eslogan ? (
                ...
                <>
                  {configuracion.nombreIglesia && (
                    <h1
                      className={`${configuracion.fontSize.titulo} font-bold mb-2`}
                  ...
                  {fondos.map((fondo) => (
                    ...
                    >
                      <div className="aspect-video bg-slate-900 relative overflow-hidden">
                        ...
                        {fondo.activo && (
                          <div className="absolute top-1 right-1 bg-emerald-500 text-white text-[10px] px-1.5 py-0.5 rounded font-medium">
                            Activo
                          </div>
                        )}
                      </div>
            ...
            <div className="p-4">
              ...
              <ul className="space-y-1.5 text-xs text-slate-500 mb-4">
                <li className="flex items-center gap-1.5">
                  <IoInformationCircle className="text-blue-400 shrink-0" />{" "}
                  Nombre e iglesia: se vaciará
                </li>
                <li className="flex items-center gap-1.5">
                  <IoInformationCircle className="text-blue-400 shrink-0" />{" "}
                  Eslogan: se vaciará
