...
import {
  FaPlus, FaTrash, FaEdit, FaSave, FaTimes, FaStop,
  FaArrowUp, FaArrowDown, FaBroadcastTower, FaToggleOn, FaToggleOff,
  FaBullhorn, FaChevronLeft, FaChevronRight, FaCheck, FaEye,
  FaPalette, FaLayerGroup,
} from "react-icons/fa";
...
const PreviewCard = ({anuncio, plt, enVivo, onClick, seleccionado}) => (
  <div
    ...
    className={`relative rounded-2xl border-2 cursor-pointer transition-all duration-300 overflow-hidden
      ${seleccionado ? plt.borderClass + " scale-[1.02] shadow-xl " + plt.glowClass
  ...
  >
    ...
    {/* Contenido */}
    <div className="relative z-10 flex flex-col items-center justify-center p-4 text-center h-full" style={{minHeight: "140px"}}>
      {!anuncio.activo && (
        <span className="absolute top-2 right-2 text-[9px] text-slate-500 bg-slate-800/80 px-1.5 py-0.5 rounded-full">Inactivo</span>
      )}
      {enVivo && (
        <span className="absolute top-2 left-2 flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full bg-orange-500/20 border border-orange-500/40 text-orange-300">
          <span className="w-1.5 h-1.5 rounded-full bg-orange-400 animate-pulse" />EN VIVO
        </span>
      )}
      {anuncio.titulo && (
    ...
    {Object.entries(PLANTILLAS).map(([id, plt]) => (
      <button
        ...
        className={`relative rounded-xl border-2 p-3 text-center transition-all overflow-hidden
          ${valor === id ? plt.borderClass + " scale-105 shadow-lg " + plt.glowClass
...
export default function Anuncios() {
  ...
  const [form, setForm]           = useState({texto: "", titulo: "", plantilla: "moderno"});
  ...
  const abrirNuevo = () => {
    ...
    setSelId(null); setEditando(false); setModoNuevo(true);
  };

  const abrirEdicion = (a) => {
    setForm({texto: a.texto, titulo: a.titulo || "", plantilla: a.plantilla || "moderno"});
    setSelId(a.id); setEditando(true); setModoNuevo(false);
  };
  ...
  return (
    <div className="bg-slate-950 text-slate-100 h-full flex flex-col overflow-hidden">
      ...
      <div className="fixed top-4 right-4 z-50 flex flex-col gap-2 pointer-events-none">
        {toasts.map(t => (
          <div key={t.id} className={`flex items-center gap-2 px-3 py-2 rounded-xl border shadow-xl text-xs min-w-[180px]
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {proyectando ? (
            <div className="flex items-center gap-1">
              <button onClick={() => proyectarIndice((indiceActual-1+activos.length)%activos.length)}
              ...
              <span className="text-[11px] text-orange-300 tabular-nums px-1 font-mono">{indiceActual+1}/{activos.length}</span>
              <button onClick={() => proyectarIndice((indiceActual+1)%activos.length)}
                disabled={activos.length<=1}
                className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 border border-white/8 flex items-center justify-center text-slate-400 hover:text-white disabled:opacity-30 transition-colors">
                <FaChevronRight className="text-[9px]" />
              </button>
              <button onClick={detenerProyeccion}
            ...
            <button onClick={iniciarProyeccion} disabled={activos.length===0}
          ...
          {anuncios.length === 0 && !formularioAbierto && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-slate-500 text-sm mb-4 max-w-xs">
                Crea anuncios con plantillas visuales para proyectarlos de forma rotativa.
              </p>
              <button onClick={abrirNuevo}
            ...
            {anuncios.map((a, idx) => {
              ...
              return (
                <div key={a.id} className="flex flex-col gap-1.5">
                  <PreviewCard
                    anuncio={a} plt={plt} enVivo={enVivo}
                  ...
                  <div className="flex items-center gap-1 px-1">
                    <button onClick={() => toggleActivo(a)} className="w-7 h-6 flex items-center justify-center text-base transition-colors hover:scale-110" title={a.activo?"Desactivar":"Activar"}>
                      {a.activo
                        ? <FaToggleOn className="text-emerald-400" />
                        : <FaToggleOff className="text-slate-600" />}
                    </button>
                    {proyectando && a.activo && (
                      <button onClick={() => proyectarIndice(activos.findIndex(x=>x.id===a.id))}
                        className="w-6 h-6 rounded-md bg-orange-500/15 hover:bg-orange-500/25 border border-orange-500/25 flex items-center justify-center text-orange-400 transition-colors" title="Proyectar ahora">
          ...
          <div className="w-72 xl:w-80 shrink-0 border-l border-slate-800 flex flex-col bg-slate-900/40 overflow-y-auto">
            <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-white">{editando ? "Editar" : "Nuevo"} anuncio</span>
              </div>
              <button onClick={() => {setModoNuevo(false); setEditando(false); setSelId(null);}}
            ...
            <div className="flex-1 p-4 space-y-4">
              {/* Título opcional */}
              <div>
                <label className="block text-[10px] text-slate-400 uppercase tracking-wider font-semibold mb-1.5">Título (opcional)</label>
                <input
                  value={form.titulo}
              ...
              {/* Texto */}
              <div>
                <label className="block text-[10px] text-slate-400 uppercase tracking-wider font-semibold mb-1.5">Texto del anuncio *</label>
                <textarea
                  autoFocus
                  value={form.texto}
              ...
              {/* Preview miniatura */}
              <div>
                <label className="block text-[10px] text-slate-400 uppercase tracking-wider font-semibold mb-2">Vista previa</label>
            ...
            <div className="px-4 py-3 border-t border-slate-800 flex gap-2">
              ...
              {editando && (
                ...
                ><FaBroadcastTower className="text-[10px]" /></button>
              )}
            </div>
          </div>
        )}
