              ...
              {resultadosPaleta.length === 0 ? (
                ...
                resultadosPaleta.map((himno, i) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${i === seleccionado ? "bg-amber-600/20 text-white" : "hover:bg-slate-800 text-slate-300"}`}
                  >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 relative min-w-0">
            ...
            <input
              ...
              className="w-full pl-7 pr-20 py-1.5 bg-slate-800 border border-slate-600/60 hover:border-slate-500 focus:border-amber-500/70 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none transition-colors"
            />
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <select
              ...
              className="bg-slate-800 border border-slate-600/60 hover:border-slate-500 focus:border-amber-500/70 rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none transition-colors"
            >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {filtrados.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2 p-3">
            {filtrados.map((himno) => (
              <div key={himno.numero} className="group relative bg-slate-800/60 hover:bg-slate-800 border border-slate-700/60 hover:border-amber-500/40 rounded-xl p-3 transition-all">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] font-bold text-amber-400/80 tabular-nums">#{himno.numero}</span>
                  <button
                ...
                <div onClick={() => handleClick(himno.numero)} className="cursor-pointer">
                  <p className="text-xs font-medium text-slate-200 leading-snug mb-2.5 line-clamp-2 min-h-[2.5rem]">{himno.titulo}</p>
                  <div className="flex items-center justify-center gap-1.5 bg-amber-600/80 hover:bg-amber-600 text-white text-[10px] font-semibold px-2 py-1.5 rounded-lg transition-colors">
            ...
            {filtrados.map((himno) => (
              <div key={himno.numero} className="flex items-center gap-3 px-3 py-2 bg-slate-900/60 hover:bg-slate-800/80 border border-slate-700/50 hover:border-slate-600/70 rounded-lg transition-colors group">
                <button onClick={() => handleClick(himno.numero)} className="flex items-center gap-3 flex-1 min-w-0 text-left">
                  <span className="shrink-0 w-9 text-center text-xs font-bold text-amber-400/70 tabular-nums">{himno.numero}</span>
