...
import {
  FaCheck, FaSave, FaBroadcastTower, FaStop, FaPalette,
  FaMagic, FaToggleOn, FaToggleOff, FaEye,
} from "react-icons/fa";
...
const DEMO_TEXTO  = "Porque de tal manera amó Dios al mundo, que ha dado a su Hijo unigénito";
const DEMO_TITULO = "Juan 3:16";

function ColorInput({label, value, onChange}) {
  return (
    <div>
      <label className="block text-[10px] text-slate-400 uppercase tracking-wider font-semibold mb-1.5">{label}</label>
...
export default function Plantillas() {
  ...
  const cargar = async () => {
    try {
      ...
      if (cfg) {
        const loaded = {
          plantillaGsapActiva:   cfg.plantillaGsapActiva    || DEFAULTS.plantillaGsapActiva,
          plantillaGsapColor1:   cfg.plantillaGsapColor1    || DEFAULTS.plantillaGsapColor1,
          plantillaGsapColor2:   cfg.plantillaGsapColor2    || DEFAULTS.plantillaGsapColor2,
          plantillaGsapColorAcc: cfg.plantillaGsapColorAcc  || DEFAULTS.plantillaGsapColorAcc,
          plantillaGsapVelocidad:cfg.plantillaGsapVelocidad || DEFAULTS.plantillaGsapVelocidad,
        };
        setConfig(loaded);
  ...
  const sincronizarLocalStorage = (cfg) => {
    ...
    if (activa) {
      localStorage.setItem("gsap-plantilla-global", JSON.stringify({
  ...
  const toggleActiva = () => {
    const nuevo = plantillaActiva ? "ninguna" : (config.plantillaGsapActiva === "ninguna" ? "revelar" : config.plantillaGsapActiva);
  ...
  const proyectarTest = () => {
    ...
    try { window.electron?.ipcRenderer?.send?.("proyectar-slide-data", slideData); } catch {}
    setProyectando(true);
  };
    ...
    <div className="bg-slate-950 text-slate-100 h-full flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 px-5 py-3 border-b border-slate-800 flex items-center justify-between bg-slate-900/40">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-semibold text-white">Plantillas GSAP</span>
          <span className={`text-[10px] px-2 py-0.5 rounded-full border font-semibold ${
            plantillaActiva
              ? "bg-emerald-500/15 border-emerald-500/30 text-emerald-400"
              : "bg-white/5 border-white/10 text-slate-500"
          }`}>{plantillaActiva ? `Activa: ${META_PLANTILLAS[config.plantillaGsapActiva]?.nombre}` : "Ninguna activa"}</span>
        </div>
        <div className="flex items-center gap-2">
          {plantillaActiva && (
            proyectando ? (
              <button onClick={detenerTest}
          ...
          <button onClick={guardar} disabled={guardando}
            ...
            {guardado ? <><FaCheck className="text-[9px]" /> Guardado</> : <><FaSave className="text-[9px]" /> Guardar</>}
          </button>
        </div>
      </div>
        ...
        <div className="w-80 xl:w-96 shrink-0 border-r border-slate-800 overflow-y-auto p-5 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Estado</h3>
              <button
                onClick={toggleActiva}
                className="flex items-center gap-2 text-sm transition-colors"
              >
                {plantillaActiva
                  ? <><FaToggleOn className="text-emerald-400 text-xl" /><span className="text-emerald-400 text-xs font-semibold">Activa</span></>
                  : <><FaToggleOff className="text-slate-600 text-xl" /><span className="text-slate-500 text-xs">Inactiva</span></>
                }
              </button>
            </div>
            <p className="text-[11px] text-slate-500 leading-relaxed">
              Cuando está activa, los anuncios proyectados usarán esta plantilla animada en vez del diseño simple.
            </p>
          </div>

          {/* Selector de plantilla */}
          <div>
            <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-wider mb-3">Plantilla</h3>
            <div className="space-y-2">
              {Object.entries(META_PLANTILLAS).map(([id, meta]) => (
                ...
                >
                  <span className={`text-xl ${config.plantillaGsapActiva === id ? "text-indigo-300" : "text-slate-500"}`}>{meta.icono}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate">{meta.nombre}</p>
          ...
          <div>
            ...
            <div className="space-y-3">
              <ColorInput label="Fondo"   value={config.plantillaGsapColor2}   onChange={v => cambiarColor("plantillaGsapColor2", v)} />
              <ColorInput label="Texto"   value={config.plantillaGsapColor1}   onChange={v => cambiarColor("plantillaGsapColor1", v)} />
              <ColorInput label="Acento"  value={config.plantillaGsapColorAcc} onChange={v => cambiarColor("plantillaGsapColorAcc", v)} />
            </div>
          </div>

          {/* Velocidad */}
          <div>
            <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-wider mb-3">Velocidad de animación</h3>
          ...
          <div className="p-3 rounded-xl bg-indigo-500/8 border border-indigo-500/20">
            <p className="text-[11px] text-indigo-300/80 leading-relaxed">
              <strong className="text-indigo-300">Tip:</strong> La plantilla activa se aplica globalmente a todo el contenido proyectado: himnos, versículos bíblicos, anuncios y temporizador.
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 min-h-0 flex items-center justify-center p-6 bg-black/40">
            <div
              ...
              style={{aspectRatio: "16/9", width: "min(100%, calc((100vh - 160px) * 16/9))", maxHeight: "calc(100vh - 160px)"}}
            >
              {plantillaActiva ? (
                <PlantillaGSAP
                ...
                <div className="w-full h-full bg-slate-900 flex flex-col items-center justify-center text-center p-8">
                  ...
                  <p className="text-slate-500 text-sm mb-2">Ninguna plantilla activa</p>
                  <p className="text-slate-600 text-xs max-w-xs">
                    Activa una plantilla y elige el diseño para ver la animación aquí
                  </p>
                </div>
              )}
              ...
              {plantillaActiva && (
                <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
                  <span className="text-[10px] bg-black/60 text-white/60 px-2 py-1 rounded-md backdrop-blur-sm">
                    {META_PLANTILLAS[config.plantillaGsapActiva]?.nombre} — Velocidad: {config.plantillaGsapVelocidad}
                  </span>
