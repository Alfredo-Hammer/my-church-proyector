...
const inputCls = "w-full px-3 py-2 bg-slate-800 border border-slate-600/60 hover:border-slate-500 focus:border-emerald-500/70 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none transition-colors";
    ...
    <div className="bg-slate-950 h-full flex flex-col overflow-hidden text-slate-100">
      ...
      <div className="shrink-0 bg-slate-900/98 backdrop-blur border-b border-slate-700/50 px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-500 hidden sm:inline">GloryView · Alfredo Hammer</span>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-3">
        <div className="max-w-5xl mx-auto flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="bg-slate-900/60 border border-slate-700/50 rounded-xl p-4">
                ...
                <div className="space-y-2">
                  {[
                    {icon: <FaPhone className="text-emerald-400 text-xs" />, label: "Teléfono", value: "+1 (941) 296 4916", sub: "9 AM – 8 PM EST"},
                    {icon: <FaEnvelope className="text-sky-400 text-xs" />, label: "Email", value: "coderhammer70@gmail.com", sub: "Respuesta en 24 h"},
                    {icon: <FaMapMarkerAlt className="text-rose-400 text-xs" />, label: "Ubicación", value: "Sarasota, FL 34234", sub: "Estados Unidos"},
                  ].map(({icon, label, value, sub}) => (
                    <div key={label} className="flex items-center gap-3 px-3 py-2 bg-slate-800/60 border border-slate-700/40 rounded-lg">
              ...
              <div className="bg-slate-900/60 border border-slate-700/50 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-800">
                  ...
                  <span className="text-xs font-semibold text-slate-300">Redes Sociales</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    {icon: <FaGithub className="text-slate-300" />, label: "GitHub"},
                    {icon: <FaLinkedin className="text-blue-400" />, label: "LinkedIn"},
                    {icon: <FaTwitter className="text-sky-400" />, label: "Twitter"},
                    {icon: <FaInstagram className="text-pink-400" />, label: "Instagram"},
                  ].map(({icon, label}) => (
                    <a key={label} href="#"
              ...
              <div className="bg-slate-900/60 border border-slate-700/50 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-800">
                  ...
                  <span className="text-xs font-semibold text-slate-300">Testimonios</span>
                </div>
                <div className="space-y-2">
                  {[
                    {texto: "Transformaron mi visión en realidad. Una experiencia increíble.", autor: "Cliente Satisfecho"},
                    {texto: "Altamente profesional y comprometido. ¡Muy recomendados!", autor: "Otro Cliente"},
                  ].map(({texto, autor}) => (
                    <div key={autor} className="px-3 py-2 bg-slate-800/60 border border-slate-700/40 rounded-lg">
                      <p className="text-xs text-slate-400 italic leading-relaxed">"{texto}"</p>
            ...
            <div className="lg:col-span-2">
              <div className="bg-slate-900/60 border border-slate-700/50 rounded-xl p-4 h-full flex flex-col">
                <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-800 shrink-0">
                  ...
                  <span className="text-xs font-semibold text-slate-300">Envíanos un mensaje</span>
                </div>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-3 flex-1">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input type="email" name="email" value={formData.email}
                        onChange={handleInputChange} placeholder="tu@email.com"
                        className={inputCls} required />
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <textarea name="mensaje" value={formData.mensaje}
                      onChange={handleInputChange} placeholder="Escribe tu mensaje aquí…"
          ...
          <p className="text-center text-slate-600 text-xs py-2">
            ...
            <span className="text-emerald-500">Desarrollado por Alfredo Hammer</span>
          </p>
        </div>
      </div>
    </div>
