        ...
        {toasts.map((t) => (
          <div key={t.id}
            className={`flex items-center gap-3 px-4 py-2.5 rounded-xl bg-slate-900/98 border shadow-2xl text-sm min-w-[240px] pointer-events-auto
              ${t.type === "success" ? "border-emerald-500/30 border-l-2 border-l-emerald-500"
              : t.type === "error"   ? "border-red-500/30 border-l-2 border-l-red-500"
            ...
            <button onClick={() => setToasts((p) => p.filter((x) => x.id !== t.id))} className="text-white/30 hover:text-white/70">
          ...
          >
            ...
            <div className="max-h-80 overflow-y-auto py-1">
              {resultadosPaleta.length === 0 ? (
                <p className="text-slate-500 text-sm text-center py-10">Sin resultados</p>
              ) : (
                resultadosPaleta.map((himno, i) => (
                  ...
                  >
                    <span className={`shrink-0 w-8 h-7 flex items-center justify-center rounded-lg text-[11px] font-bold tabular-nums
      ...
      <div className="shrink-0 z-30 bg-[#0d1117]/95 backdrop-blur border-b border-white/[0.06] px-3 py-2.5">
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2 shrink-0">
            ...
            <span className="text-sm font-semibold text-white hidden sm:inline">Himnario Moravo</span>
          </div>
          ...
          >
            ...
            <option value="titulo"    className="bg-slate-900">Título A-Z</option>
            <option value="favoritos" className="bg-slate-900">Favoritos</option>
          </select>
          ...
          <div className="flex items-center gap-0.5 bg-white/5 border border-white/8 rounded-lg p-0.5 shrink-0">
            <button onClick={() => setVistaGrid(false)}
              className={`w-7 h-7 rounded-md flex items-center justify-center transition-all ${!vistaGrid ? "bg-emerald-600 text-white" : "text-slate-500 hover:text-slate-300"}`}
              title="Lista"><FaList className="text-xs" /></button>
            <button onClick={() => setVistaGrid(true)}
              className={`w-7 h-7 rounded-md flex items-center justify-center transition-all ${vistaGrid ? "bg-emerald-600 text-white" : "text-slate-500 hover:text-slate-300"}`}
              title="Grid"><FaTh className="text-xs" /></button>
          </div>

          <div className="hidden md:flex items-center gap-1.5 shrink-0 text-[11px] text-slate-600">
            <span className="tabular-nums">{filtrados.length}<span className="text-slate-700">/{himnos.length}</span></span>
            {favoritos.size > 0 && (
              <span className="flex items-center gap-0.5 text-rose-500/60">
                <FaHeart className="text-[8px]" />{favoritos.size}
              </span>
            )}
          </div>
        </div>
        ...
        {filtrados.length === 0 ? (
              ...
              return (
                <div key={himno.numero}
                  ...
                  <div className="absolute top-0 right-0 w-24 h-24 rounded-bl-3xl pointer-events-none"
                    style={{background: esFav ? "radial-gradient(circle at 100% 0%, rgba(244,63,94,0.08) 0%, transparent 70%)" : "radial-gradient(circle at 100% 0%, rgba(16,185,129,0.06) 0%, transparent 70%)"}} />
                  <span className="absolute top-2.5 right-3 text-3xl select-none pointer-events-none"
                    style={{color: esFav ? "rgba(244,63,94,0.12)" : "rgba(16,185,129,0.10)"}}>♪</span>
                  ...
                  <div className="flex-1 p-4 flex flex-col">
                    ...
                    <Link to={`/himno/${himno.numero}`} className="flex-1 flex flex-col">
          ...
          <div className="flex flex-col gap-px px-3 py-3">
            {listaConDivisores.map((item, idx) => {
              if (item.tipo === "rango") {
                return (
                  <div key={`rango-${item.label}`} className={`flex items-center gap-3 px-1 ${idx === 0 ? "pt-0 pb-2" : "pt-4 pb-2"}`}>
                  ...
                  <Link to={`/himno/${himno.numero}`} className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className={`shrink-0 w-11 h-11 rounded-xl flex flex-col items-center justify-center transition-all
                      ...
                      <span className={`text-[9px] uppercase tracking-wider leading-none mb-0.5
                        ${esFav ? "text-rose-500/50" : "text-emerald-500/40"}`}>nro</span>
