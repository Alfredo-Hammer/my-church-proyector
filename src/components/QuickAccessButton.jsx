...
const QuickAccessButton = ({icon: Icon, label, onClick, color = "emerald"}) => {
  ...
  return (
    <motion.button
      ...
      whileTap={{scale: 0.95}}
      onClick={onClick}
      className={`w-full rounded-xl p-5 border backdrop-blur-sm ${colorClass} transition-all flex flex-col items-center justify-center gap-3 hover:shadow-lg min-h-[120px]`}
    >
      {Icon && <Icon className="text-3xl md:text-4xl" />}
      <span className="text-sm md:text-base font-semibold text-center text-white">
        {label}
      </span>
    </motion.button>
  );
