...
const precargarImagen = (url) => new Promise((resolve, reject) => {
...
const leerGsapGlobal = () => {
  try { return JSON.parse(localStorage.getItem("gsap-plantilla-global") || "null"); }
  catch { return null; }
};
...
function modoReducer(_, action) {
  switch (action.type) {
    ...
    case "LIMPIAR":      return action.destino ?? "bienvenida";
...
const SLIDE_TEXT_SHADOW_TITLE   = "0 2px 8px rgba(0,0,0,0.9), 0 0 30px rgba(0,0,0,0.5)";
const SLIDE_TEXT_SHADOW_CONTENT = "0 1px 4px rgba(0,0,0,0.9)";
const TEXT_QUALITY = {textRendering:"optimizeLegibility", WebkitFontSmoothing:"antialiased"};
      ...
      {sl.title && (
        <motion.h1
          ...
          style={{
            ...
            textShadow:     sl.textShadow !== false ? SLIDE_TEXT_SHADOW_TITLE : "none",
            ...TEXT_QUALITY,
          }}
      ...
      {sl.content && (
        <motion.div
          ...
          style={{
            ...
            textShadow:     sl.textShadow !== false ? SLIDE_TEXT_SHADOW_CONTENT : "none",
            ...TEXT_QUALITY,
          }}
          initial={{opacity:0, y:30}}
          animate={{opacity:1, y:0}}
        ...
        >
          {sl.content.split("\n").map((line, i) => (
            <p key={`${i}-${line.slice(0,10)}`} className="mb-3">{line}</p>
          ))}
        </motion.div>
      )}
    </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        color:            sl.textColor        || "#ffffff",
        backgroundImage:  sl.backgroundImage ? `url(${optimizarUrlImagen(sl.backgroundImage)})` : "none",
...
const Proyector = () => {
  ...
  useEffect(() => {
    ...
    prevCursorRef.current = {html: h?.style?.cursor||"", body: b?.style?.cursor||""};
  ...
  useEffect(() => {
    ...
    prevStylesRef.current = {
      htmlBg: h?.style?.backgroundColor||"", bodyBg: b?.style?.backgroundColor||"",
      htmlColor: h?.style?.color||"",         bodyColor: b?.style?.color||"",
      htmlOverflow: h?.style?.overflow||"",   bodyOverflow: b?.style?.overflow||"",
    };
    const apply = (el) => { el.style.backgroundColor="#000"; el.style.color="#fff"; el.style.overflow="hidden"; };
    if (h?.style) apply(h);
    if (b?.style) apply(b);
    return () => {
      const p = prevStylesRef.current;
      if (h?.style) { h.style.backgroundColor=p.htmlBg; h.style.color=p.htmlColor; h.style.overflow=p.htmlOverflow; }
      if (b?.style) { b.style.backgroundColor=p.bodyBg; b.style.color=p.bodyColor; b.style.overflow=p.bodyOverflow; }
    };
  }, []);
  ...
  useEffect(() => { tieneMultiplesMonitoresRef.current = tieneMultiplesMonitores; }, [tieneMultiplesMonitores]);
  ...
  const limpiarProyector = useCallback(() => {
    const reset = () => {
      setParrafo(""); setTitulo(""); setNumero("");
      setMultimediaActiva(null); setTimerData(null);
      himnoActualRef.current    = "";
      tituloMostradoRef.current = false;
      setMostrarTituloHimno(true);
      setPresentacionActiva(null); setSlideData(null); setSlideActual(0);
  ...
  useEffect(() => { limpiarRef.current = limpiarProyector; }, [limpiarProyector]);
  ...
  const handleMostrarVersiculo = useCallback((_, {parrafo: p, titulo: t, numero: n, origen}) => {
    if (origen === "clear") { limpiarProyector(); return; }
    setMostrarTituloHimno(true);
    himnoActualRef.current    = "";
    tituloMostradoRef.current = false;
    setMultimediaActiva(null);
    if (origen === "biblia") {
      setTitulo(`${t} ${n}`); setParrafo(p || "No disponible"); setNumero("");
    } else {
      setTitulo(t || "Himno"); setParrafo(p || "No disponible"); setNumero(n || "");
    }
    setShowContent(true);
    dispatchModo({type:"HIMNO"});
  }, [limpiarProyector]);
  ...
  const handleMostrarMultimedia = useCallback((_, mediaData) => {
    ...
    if (!mediaData.url.startsWith("http://") && !mediaData.url.startsWith("https://")) return;
    setParrafo(""); setTitulo(""); setNumero("");
  ...
  const handleLimpiarProyector = useCallback(() => limpiarProyector(), [limpiarProyector]);
  ...
  const handleDetenerPresentacion = useCallback(() => {
    setPresentacionActiva(null); setSlideActual(0);
    dispatchModo({type:"BIENVENIDA"});
  }, []);
  ...
  const handleProyectarSlideData = useCallback((_, sd) => {
    ...
    setTitulo(""); setParrafo(""); setNumero("");
    setMultimediaActiva(null);
  ...
  const handleConfigurarMonitores = useCallback((_, data) => {
    ...
    dispatchModo({type: data.tieneMultiplesMonitores ? "BIENVENIDA" : "ESPERA"});
  }, []);
  ...
  useIpcListener("actualizar-multimedia-activa", handleActualizarMultimediaActiva);
  ...
  useEffect(() => {
    try { window.electron?.send?.("proyector-ready", {ts: Date.now()}); } catch { /* noop */ }
  }, []);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        if (list?.length) {
          setPresentaciones(list.map(p => ({
            id: p.id, name: p.nombre,
            slides: Array.isArray(p.slides) ? p.slides : JSON.parse(p.slides || "[]"),
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.key === "proyector-slide-data") {
        try { handleProyectarSlideData(null, JSON.parse(e.newValue)); } catch { /* noop */ }
      }
      if (e.key === "gsap-plantilla-global") {
        try { setGsapGlobal(e.newValue ? JSON.parse(e.newValue) : null); } catch { /* noop */ }
      }
    };
  ...
  useEffect(() => {
    ...
    if (!bgImage) { setImagenCargando(false); return; }
  ...
  useEffect(() => {
    const onKey = (e) => {
      switch (e.key) {
        case "Escape": limpiarProyector(); break;
        case "F5":     reloadConfig(); background.reload(); break;
        case "F6":     if (background.usandoFondoDefecto) background.seleccionarVideoDefecto(); break;
        case "F7":     setEfectosActivos(p => !p); break;
        case "F8":     setTransicionSuave(p => !p); break;
        case " ":
  ...
  const isVideoBackground = modo === "multimedia" && multimediaActiva?.tipo === "video";
  ...
  return (
    <ErrorBoundary>
      <motion.div
        ...
        style={{
          backgroundColor:"#000",
          imageRendering:"crisp-edges", textRendering:"optimizeLegibility",
          WebkitFontSmoothing:"antialiased", MozOsxFontSmoothing:"grayscale",
          backfaceVisibility:"hidden", perspective:"1000px", cursor:"none",
        }}
      >
                ...
                >
                  {background.fondoPrevio.tipo === "imagen" ? (
                    <div className="w-full h-full bg-cover bg-center bg-no-repeat"
                      style={{backgroundImage:`url(${background.fondoPrevio.url})`, filter:"contrast(1.1) brightness(1.05)", backfaceVisibility:"hidden"}} />
                  ) : (
                    <video className="w-full h-full object-cover bg-black"
                      src={background.fondoPrevio.url} autoPlay loop muted playsInline
            ...
            <AnimatePresence>
              ...
              >
                {background.fondoActivo?.tipo === "imagen" ? (
                  <div className="w-full h-full bg-cover bg-center bg-no-repeat"
                    style={{backgroundImage:`url(${background.fondoActual})`, filter:"contrast(1.1) brightness(1.05)", backfaceVisibility:"hidden"}} />
                ) : (
                  <video className="w-full h-full object-cover bg-black"
                    src={background.fondoActual} autoPlay loop muted playsInline preload="auto"
                    style={{filter:"contrast(1.05) brightness(1.02)", backfaceVisibility:"hidden"}}
                    onError={() => { if (!background.usandoFondoDefecto) background.seleccionarVideoDefecto(); }}
                  />
                )}
              </motion.div>
            </AnimatePresence>
        ...
        <ParticleBackground
          mode={modo}
          isActive={efectosActivos && modo !== "multimedia" && modo !== "slide"}
        />
        ...
        <AnimatePresence mode="wait">
          {parrafo && showContent && modo === "himno" && (
            gsapGlobal?.plantillaId && gsapGlobal.plantillaId !== "ninguna" ? (
              <div key={`gsap-${gsapGlobal.plantillaId}`} className="absolute inset-0 z-10 w-full h-screen">
                <PlantillaGSAP
                  titulo={titulo?.trim() || ""} texto={parrafo}
          ...
          {modo === "presentacion" && presentacionActiva && showContent && (
            ...
            >
              {presentacionActiva.slides?.[slideActual] && (() => {
                ...
                const bgStyle = bgImg && !imagenesConError.has(bgImg)
                  ? `url(${optimizarUrlImagen(bgImg)})` : "none";
                return (
                  <SlideWrapper sl={{...sl, backgroundImage: bgStyle !== "none" ? bgImg : null}}
                    ...
                    {imagenCargando && (
                      <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-5">
                        <div className="flex items-center space-x-3 text-white">
                          ...
                          <span className="text-lg font-medium">Cargando imagen...</span>
                        </div>
                      </div>
                    )}
        ...
        {modo === "temporizador" && timerData && showContent && (
          <div className="absolute inset-0 z-40">
            <TimerDisplay
              segundos={timerData.segundos} total={timerData.total}
              mensaje={timerData.mensaje}   terminado={timerData.terminado}
            />
          </div>
        )}
        ...
        {/* ── Slide individual ── */}
        <AnimatePresence mode="wait">
          {modo === "slide" && slideData?.slide && showContent && (
            slideData.slide?.gsapTemplate ? (
              <div key={`gsap-slide-${slideData.slide.id}`} className="absolute inset-0 z-30">
                <PlantillaGSAP
              ...
              >
                <SlideWrapper
                  sl={slideData.slide}
                  counter={slideData.presentation
